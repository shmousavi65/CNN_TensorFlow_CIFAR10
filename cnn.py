...
class Conv_NNet(object):
    ...
    @staticmethod
    def vectorizing(x):
        """ Return the vectorized version of input tensor.
        ...
        return tf.reshape(x, [-1, x_length])
    ...
    def forward(self,inp_batch, keep_pro):
        ...
        with tf.variable_scope('output_layer', reuse=tf.AUTO_REUSE):
            output = self.out(inp=layer, num_of_outputs=self.num_of_outputs)
        return output
    ...
    def accuracy(output, labels):
        ...
        """
        correct_pred = tf.equal(tf.argmax(output, axis=1), tf.argmax(labels, axis=1))
        return tf.reduce_mean(tf.cast(correct_pred, dtype=tf.float32))
    ...
    def test(self, images, labels, label_names):
        ...
        print("Prediction Accuracy: {:.3f}".format(np.mean(correct_prediction)))
